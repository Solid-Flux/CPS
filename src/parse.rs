...
impl<T> MyExtendable for syn::Result<T>
{
    fn add_message<S: Display>(self, input: ParseStream, msg: S) -> Self {
        self.map_err(|e| {
            input.error(format!("{}: {}", msg, e.to_string()))
        })
    }
}
...
fn parse_paren(input: ParseStream) -> syn::Result<(Paren, TokenStream)> {
...
impl ToTokens for MacroVariableIdentifier {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        let Self {
            dollar_sign, identifier, colon, macro_frag_spec
        } = self.clone();
        *tokens = quote! {
            #tokens
...
impl ToTokens for MacroRepetition {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        let Self {
            dollar_sign, sub_matches, rep_sep, rep_op, ..
        } = self.clone();
        *tokens = quote! {
            #tokens
...
fn parse_ident_or_recursive(input_fork: &ParseBuffer) -> Option<MacroMatch> {
    if input_fork.peek(Token![$]) {
        if input_fork.peek2(Ident::peek_any) { // Identifier
            return input_fork.parse().map(|i| MacroMatch::Identifier(i)).ok()
        } else if input_fork.peek2(Paren) { // Repetition
...
impl Parse for MacroMatch {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        return match parse_ident_or_recursive(&input_fork) {
            ...
            None => { // On fail to parse, assume Token
...
impl ToTokens for MacroMatch {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        match self {
            ...
            MacroMatch::Group(d, t) => {
                delimiter_to_tokens(&d, &t).to_tokens(tokens)
            },
...
impl MacroMatcher {
    pub fn build_output_clone(&self) -> TokenStream {
        let mapped = self.matches.iter().map(|m| m.build_output_clone()).collect::<Vec<TokenStream>>();
        quote! {
            #(#mapped)*
        }
    }
...
impl LetBinding {
    fn parse_helper(input: ParseStream) -> syn::Result<Self> {
        ...
        let macro_invocation = input.parse()?;
